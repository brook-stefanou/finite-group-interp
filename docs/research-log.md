...
- Ran the energy instrument along the trajectory for the matched pair at d_model=64 — the regime where the learnability gap is real and the post reports it — at full sample size, split by Frobenius–Schur type (Dih = 4 dim-1 + 25 real dim-2; Dic = 4 dim-1 + 12 real + 13 quaternionic dim-2). Strict grok rule (manifest status completed + a grokked_step checkpoint + final test_acc ≥ 0.99): of 276 pair runs (all confirmed d=64), 241 strict-grokked (D52 129 / Dic26 112), 35 non-grokked, 0 excluded for sparse checkpoints → n = 105 matched-grokked seeds. (a) Matched seed-for-seed, the dicyclic's global energy-concentration onset (50% of final energy-above-uniform) is later than the dihedral's in 90/105 seeds; median onset Dih 8.0k vs Dic 19.0k (gap +8.6k epochs) — it sits in near-uniform/memorised energy longer before concentrating.
- (b) Within each dicyclic run (same optimiser/seed, the cleanest control) the quaternionic dim-2 sector is the *last* to concentrate: at-or-after the real dim-2 sector in 94/105 runs (strictly later in 90); median onset relative to grok is real-2d −16.4k vs quaternionic −4.9k (gap +10.5k epochs) — the real sector locks in well before grok, the quaternionic last to light up. Turns "dicyclic is slower" into a rep-type story tied to the real-vs-quaternionic axis — correlational, n=105. (A d=256, n=4 pilot first suggested this: 4/4 on (a), 3/4-plus-tie on (b).) (`scripts/analyze_learnability_trajectory_d64.py` → `docs/figures/learnability-trajectory-d64.png`; numbers in gitignored scratch.)
- **R² gap flips from null to significant on the big sweep.** Reran gap/coset/learnability on 138 transformer seeds/group (105 matched) + 31 FC seeds/group. The gap now separates the pair — larger for the dihedral than the dicyclic (0.063 vs 0.038, Welch p=0.0019; FC 0.021 vs 0.012, p=0.0005) — vs the old n=27 null (p=0.25). Direction is opposite to my prediction and unexplained so far. Coset excess stays null; learnability split holds.
- **The causal coset-ablation is degenerate at d_model=64.** The coset-separating residual subspace spans the whole residual (orthogonal complement ≈1e-14, unanimous over 30 checkpoints): coset info dimension R=56 of 104 (separating ceiling 89) exceeds the 64-wide residual, so the structure no coset can express can't be ablated. Logged as an evidence-backed limitation, not a missing experiment.
- **At d_model=256 the computation looks coset-expressible.** Retrained the pair 4× wider (8 runs, all grok); the coset subspace (rank 96 of 256) now leaves a complement. Removing the non-coset complement costs ~0 test acc (every seed, both groups); removing the coset subspace is catastrophic — so the model doesn't use the rep structure cosets can't express even given the width. Pushes against the rep-theory lean (caveat: rules out the uniquely-faithful structure, not a coset mechanism, and it's wider than the post's d=64).
- **Width also narrows the learnability gap (suggestive, n=4/group).** Wide runs grok far faster (3.6k–15k vs ~18k/38k medians at d=64) and the dicyclic groks about as fast as the dihedral, so "dicyclic is slower" may be capacity-specific. Decision: keep the post at d=64, carry the width sweep + learnability-at-width to the follow-up; softened the dim-3/dim-4 claim to preliminary.
