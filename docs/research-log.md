...
- Ran the energy instrument along the trajectory for the 4 matched wide-256 seeds, split by Frobenius–Schur type (Dih = 4 dim-1 + 25 real dim-2; Dic = 4 dim-1 + 12 real + 13 quaternionic dim-2). Even though these wide runs grok at comparable speed, the instrument still resolves the lag: matched seed-for-seed, the dicyclic's energy-concentration onset (50% of final energy-above-baseline) is ~1.0–1.5k epochs later than the dihedral's in all 4/4 seeds — it sits in near-uniform/memorised energy longer before concentrating.
- Within each dicyclic run (same optimiser/seed, the cleanest control) the quaternionic dim-2 sector is the *last* to concentrate: the real dim-2 onset precedes the quaternionic one in 3/4 seeds (tie in the 4th), median ~1k epochs later relative to grok. Turns "dicyclic is slower" into a rep-type story tied to the real-vs-quaternionic axis — correlational, n=4. (`scripts/analyze_learnability_trajectory.py` → `docs/figures/learnability-trajectory.png`.)
