...
class FiniteGroup(BaseModel):
    ...
    def _closure(self, gens: list[Element]) -> frozenset[Element]:
        """The subgroup generated by ``gens``: close under the operation.

        In a finite group, closure under multiplication suffices -- inverses
        appear automatically (g^-1 = g^(order(g)-1)).
        """
        identity = self.elements[self._check_identity()]
        elems: set[Element] = set(gens) | {identity}
        changed = True
        while changed:
            changed = False
            for x in list(elems):
    ...
    def subgroups(self) -> list[list[Element]]:
        ...
        size then element index. Cheap for the project's order range (<~350):
        generate every cyclic subgroup, then close the collection under joins.
        """
        subs: set[frozenset[Element]] = {self._closure([g]) for g in self.elements}
        changed = True
        while changed:
            changed = False
            current = list(subs)
            for i in range(len(current)):
                for j in range(i + 1, len(current)):
                    joined = self._closure(list(current[i] | current[j]))
        ...
        def as_list(s: frozenset[Element]) -> list[Element]:
            rest = sorted((e for e in s if e != identity), key=lambda e: idx[e])
            return [identity, *rest]

        return sorted(
            (as_list(s) for s in subs),
            key=lambda lst: (len(lst), [idx[e] for e in lst]),
        )
